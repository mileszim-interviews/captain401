...
export function findDate(date, dateList) {
  ...
  for (let i = 0; i < dateList.length; i++) {
    ...
    if (date > currentDate) {    // We want a date in the future, so check:
      if (i < dateList.length) { // - do we have more dates in our list?
    ...
    if (date < currentDate) {    // We asked for a date earlier than the current one, so check:
      if (i > 0) {               // - are we past the first date?
        return dateList[i-1];    // -- then set it to the previous date
      }                          //
      return dateList[0];        // otherwise return the first date in the list (keeping it simple)
    }
  }
}
